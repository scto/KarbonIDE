...
/**
 * Search text in editor. Note that editor searches text in another thread, so results may not be
 * available immediately. Also, the searcher does not match empty text. For example, you will never
 * match a single empty line by regex '^.*$'. What's more, zero-length pattern is not permitted. The
 * searcher updates its search results automatically when editor text is changed, even after {@link
