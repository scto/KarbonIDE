/**
 * Copyright (c) 2015-2022 Angelo ZERR and others. This program and the accompanying materials are
 * made available under the terms of the Eclipse Public License 2.0 which is available at
...
public final class TextUtils {
  ...
  public static String normalizeIndentation(
      final String text, final int tabSize, final boolean insertSpaces) {
    int firstNonWhitespaceIndex = firstNonWhitespaceIndex(text);
    if (firstNonWhitespaceIndex == -1) {
      firstNonWhitespaceIndex = text.length();
    }
    return normalizeIndentationFromWhitespace(
            text.substring(0, firstNonWhitespaceIndex), tabSize, insertSpaces)
  ...
  /**
   * Returns the start of the string at the offset in the text. If the string is not in the text at
   * the offset, returns -1.
   *
   * <p>Example:
   *
   * <pre>
  ...
  public static int startIndexOfOffsetTouchingString(
      final String text, final int offset, final String string) {
  ...
  public static String getIndentationFromWhitespace(
      final String whitespace, final int tabSize, final boolean insertSpaces) {
    final var tab = "\t"; // $NON-NLS-1$
    int indentOffset = 0;
    boolean startsWithTab = true;
    boolean startsWithSpaces = true;
    final String spaces = insertSpaces ? " ".repeat(tabSize) : "";
  ...
  /**
   * Returns the first offset greater than <code>offset</code> and smaller than <code>end</code>
  ...
  /**
   * Determines if all the characters at any offset of the specified document line are the
   * whitespace characters.
   *
   * @param doc the document to search in
   * @param line zero-based document line number
   * @return <code>true</code> if all the characters of the specified document line are the
   *     whitespace characters, otherwise returns <code>false</code>
   */
