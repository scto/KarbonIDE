...
public class CachedIndexer implements Indexer, ContentListener {
  ...
  void findIndexBackward(@NonNull CharPosition start, int index, @NonNull CharPosition dest) {
    ...
    while (workIndex > index) {
      ...
      if (workLine != -1) {
        ...
        // Reached the start of text,we have to use findIndexForward() as this method can not handle
        // it
   ...
   */
  @VisibleForTesting
  void findLiCoForward(
      @NonNull CharPosition start, int line, int column, @NonNull CharPosition dest) {
    if (start.line > line) {
