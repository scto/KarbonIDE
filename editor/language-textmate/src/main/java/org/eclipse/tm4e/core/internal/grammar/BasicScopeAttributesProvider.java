...
final class BasicScopeAttributesProvider {
  ...
  BasicScopeAttributesProvider(
      final int initialLanguage, @Nullable final Map<String, Integer> embeddedLanguages) {
    this._defaultAttributes =
        new BasicScopeAttributes(initialLanguage, OptionalStandardTokenType.NotSet);
    this._embeddedLanguagesMatcher =
        new ScopeMatcher<>(defaultIfNull(embeddedLanguages, Collections.emptyMap()));
  }
  ...
  private static final class ScopeMatcher<TValue> {
    ...
    @Nullable private final Pattern scopesRegExp;
