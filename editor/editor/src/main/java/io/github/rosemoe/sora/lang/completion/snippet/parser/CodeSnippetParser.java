...
public class CodeSnippetParser {
  ...
  private boolean parseEscaped() {
    if (accept(TokenType.Backslash)) {
      var escaped =
          _accept(TokenType.CurlyClose, TokenType.Dollar, TokenType.Backslash, TokenType.Backtick);
      if (escaped == null) {
        escaped = "\\";
      }
    ...
    if (accept(TokenType.Dollar)
      ...
      if (accept(TokenType.Colon)) {
        ...
        while (!accept(TokenType.CurlyClose)) {
          if (accept(TokenType.Backslash)) {
            if ((text = _accept(TokenType.Backslash, TokenType.Dollar, TokenType.CurlyClose))
                != null) {
              sb.append(text);
            } else {
              sb.append('\\');
  ...
  private static void appendPlaceholderElement(
      @NonNull ArrayList<PlaceHolderElement> elements, @NonNull String t) {
    if (!elements.isEmpty()) {
