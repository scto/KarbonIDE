...
public final class Registry {
  ...
  public void setTheme(final IThemeSource source) throws TMException {
    try {
      this._syncRegistry.setTheme(
          Theme.createFromRawTheme(RawThemeReader.readTheme(source), _options.getColorMap()));
  ...
  public void setTheme(final Theme source) throws TMException {
    try {
      ...
      throw new TMException(
          "Loading theme from '" + source.toString() + "' failed: " + ex.getMessage(), ex);
    }
  }
  ...
  private IGrammar _loadGrammar(
    ...
    if (!_loadSingleGrammar(initialScopeName)) return null;
  ...
  private boolean _doLoadSingleGrammar(final String scopeName) {
    ...
    try {
      ...
      throw new TMException(
          "Loading grammar for scope ["
              + scopeName
              + "] from ["
              + grammarSource.getFilePath()
              + "] failed: "
              + ex.getMessage(),
          ex);
    }
