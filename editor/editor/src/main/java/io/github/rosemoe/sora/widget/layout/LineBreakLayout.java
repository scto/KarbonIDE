...
public class LineBreakLayout extends AbstractLayout {
              ...
              editor.postInLifecycle(
                  () -> {
                    if (LineBreakLayout.this.editor != editor
                        || reuseCountLocal != reuseCount.get()) {
            ...
            try {
              ...
              text.runReadActionsOnLines(
                  ...
                  (int index,
                    ...
                    var width =
                        (int) measurerLocal.measureText(line, 0, line.length(), shadowPaint);
                    if (shouldRun()) {
          ...
          widthMaintainer.get(startLine)
              - (int)
                  measurer.measureText(
                      deletedContent, 0, endColumn - startColumn, editor.getTextPaint()));
    } else {
  ...
  public long getCharPositionForLayoutOffset(float xOffset, float yOffset) {
    ...
    int res =
        BidiLayout.horizontalIndex(editor, this, text, line, 0, text.getColumnCount(line), xOffset);
  ...
  public float[] getCharLayoutOffset(int line, int column, float[] dest) {
    ...
    dest[1] =
        BidiLayout.horizontalOffset(editor, this, text, line, 0, text.getColumnCount(line), column);
    return dest;
  }
  ...
  static class LineBreakLayoutRowItr implements RowIterator {
    ...
    LineBreakLayoutRowItr(
        @NonNull Content text, int initialRow, @Nullable SparseArray<ContentLine> preloadedLines) {
