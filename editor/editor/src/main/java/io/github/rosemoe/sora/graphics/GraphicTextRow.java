    ...
    while (regionItr.hasNextRegion() && currentPosition < advance) {
      ...
      int res = -1;
      {
            ...
            if (lastStart != i) {
              int idx =
                  paint.findOffsetByRunAdvance(
                      text, lastStart, i, advance - currentPosition, quickMeasureMode);
              currentPosition +=
                  paint.measureTextRunAdvance(
                      chars, lastStart, idx, regionStart, regionEnd, quickMeasureMode);
              if (idx < i) {
                res = idx;
                break;
              } else {
        ...
        if (res == -1) {
          int idx =
              paint.findOffsetByRunAdvance(
                  text, lastStart, regionEnd, advance - currentPosition, quickMeasureMode);
