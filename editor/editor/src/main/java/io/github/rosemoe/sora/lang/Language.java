...
public interface Language {
  ...
  /**
   * Throw {@link CompletionCancelledException} exceptions from {@link ContentReference} and {@link
   * CompletionPublisher}.
   *
   * <p>Set thread's flag for abortion.
   */
   ...
   * @throws io.github.rosemoe.sora.lang.completion.CompletionCancelledException This thread can be
   *     abandoned by the editor framework because the auto-completion items of this invocation are
   *     no longer needed by the user. This can either be thrown by {@link ContentReference} or
   *     {@link CompletionPublisher}. How the exceptions will be thrown is according to your
  ...
  /**
   * Get newline handlers of this language. This method is called each time the user types a single
   * character (or a single code point) and some text is currently selected.
   *
   * <p>Pay attention to the performance as this method is called frequently
   *
   ...
   *
   * <p>When called, you should stop your resource-taking actions and remove any reference of editor
   * or other objects related to editor (such as references to text in editor) to avoid memory leaks
   * and resource waste.
   */
  @UiThread
  void destroy();
