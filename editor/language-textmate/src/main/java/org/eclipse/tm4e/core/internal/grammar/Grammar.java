 ...
 * license - Angelo Zerr <angelo.zerr@gmail.com> - translation and adaptation to Java - Fabio
 * Zadrozny <fabiofz@gmail.com> - Not adding '\n' on tokenize if it already finished with '\n'
 */
...
public final class Grammar implements IGrammar, IRuleFactoryHelper {
  ...
  @Nullable private RuleId _rootId;
  ...
  @Nullable private List<Injection> _injections;
  ...
  @Nullable private final BalancedBracketSelectors balancedBracketSelectors;
  ...
  public Grammar(
    ...
    this._basicScopeAttributesProvider =
        new BasicScopeAttributesProvider(initialLanguage, embeddedLanguages);
  ...
  private void collectInjections(
      ...
      final IRawRule rule,
      final IRuleFactoryHelper ruleFactoryHelper,
      final IRawGrammar grammar) {
    final var matchers = Matcher.createMatchers(selector);
    final var ruleId =
        RuleFactory.getCompiledRuleId(rule, ruleFactoryHelper, this._grammar.getRepository());
  ...
  private List<Injection> _collectInjections() {
    final var grammarRepository =
        new IGrammarRepository() {
          @Override
    ...
    if (grammar != null) {
      ...
      if (injectionScopeNames != null) {
        injectionScopeNames.forEach(
            injectionScopeName -> {
              final var injectionGrammar =
                  Grammar.this.getExternalGrammar(injectionScopeName, null);
  ...
  public Rule getRule(final RuleId ruleId) {
    ...
    if (rule == null) {
      throw new IndexOutOfBoundsException(
          "No rule with index "
              + ruleId.id
              + " found. Possible values: 0.."
              + this._ruleId2desc.size());
    }
    return rule;
  }
  ...
  @Override
  @Nullable
  public IRawGrammar getExternalGrammar(
      final String scopeName, @Nullable final IRawRepository repository) {
  ...
  @Override
  public ITokenizeLineResult<IToken[]> tokenizeLine(
  ...
  @Override
  public ITokenizeLineResult<int[]> tokenizeLine2(
      final String lineText,
      @Nullable final IStateStack prevState,
      @Nullable final Duration timeLimit) {
  ...
  private <T> T _tokenize(
      ...
      rootId =
          this._rootId =
              RuleFactory.getCompiledRuleId(
                  this._grammar.getRepository().getSelf(), this, this._grammar.getRepository());
    ...
    if (prevState == null || prevState == StateStack.NULL) {
      ...
      if (rootScopeName != null) {
        scopeList =
            AttributedScopeStack.createRootAndLookUpScopeName(rootScopeName, defaultMetadata, this);
      } else {
    ...
    final var lineTokens =
        new LineTokens(emitBinaryTokens, lineText, _tokenTypeMatchers, balancedBracketSelectors);
    final var r =
