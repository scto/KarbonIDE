...
object MenuClickHandler {
    ...
    fun handle(activity: MainActivity, menuItem: MenuItem): Boolean {
        ...
        when (id) {
            ...
            Id.action_print -> {
                Printer.print(activity, editorFragment?.editor?.text.toString())
                return true
            }
            ...
            Id.share -> {
                rkUtils.shareText(activity, editorFragment?.editor?.text.toString())
                return true
            }
            ...
            Id.suggestions -> {
                editorFragment
                    ?.editor
                    ?.showSuggestions(editorFragment.editor?.isShowSuggestion()?.not() == true)
                return true
            }
                        ...
                        when (v.id) {
                            pull -> {
                                ...
                                DefaultScope.launch(Dispatchers.IO) {
                                    try {
                                        val gitRoot = FileManager.findGitRoot(editorFragment?.file)
                                        if (gitRoot != null) {
                            ...
                            push -> {
                                ...
                                if (gitRoot != null) {
                                    ...
                                    view.findViewById<LinearLayout>(Id.mimeTypeEditor).visibility =
                                        View.VISIBLE
                                    val branchedit =
                                        view.findViewById<EditText>(Id.name).apply {
                                            hint = getString(R.string.git_branch)
                                            setText(git.repository.branch)
                                        }
                                    val commitedit =
                                        view.findViewById<EditText>(Id.mime).apply {
