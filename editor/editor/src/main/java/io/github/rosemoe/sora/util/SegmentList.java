...
public class SegmentList<T> extends AbstractList<T> {
  ...
  private FindResult<T> getSegment(int index) {
    ...
    for (int i = 0, j = segments.size() - 1; i <= j; i++, j--) {
      ...
      block = backBlock;
      if ((index >= backOffset && index < backOffset + block.size())
          || (j == segments.size() - 1 && index == length)) {
  ...
  protected void removeRange(int fromIndex, int toIndex) {
    ...
    if (fromIndex < 0 || toIndex > length)
      throw new IndexOutOfBoundsException(
          "start = " + fromIndex + ", end = " + toIndex + ", length = " + size());
    ...
    while (toIndex - offset > 0 && index < segments.size()) {
      ...
      if (fromIndex <= offset && toIndex >= offset + segLength) {
        ...
        var sub =
            seg.subList(Math.max(fromIndex - offset, 0), Math.min(toIndex - offset, segLength));
        sub.clear();
        index++;
      }
      offset += segLength;
      if (index < segments.size()) seg = segments.get(index);
    }
    mergeSegment(index - 1, index);
  ...
  private void adjustElements(int segIdx, Segment<T> mutCur) {
    if (segIdx > 0) {
      ...
      if (pre.isMutable()
          && pre.size() <= segmentCapacity * 4 / 5
          && mutCur.size() > segmentCapacity * 4 / 5) {
