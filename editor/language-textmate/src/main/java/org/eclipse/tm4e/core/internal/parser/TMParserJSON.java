 ...
 *
 * <p>Contributors: Sebastian Thomschke (Vegard IT) - initial implementation
 */
...
public class TMParserJSON implements TMParser {
  ...
  @Override
  public final <T extends PropertySettable<?>> T parse(
      final Reader source, final ObjectFactory<T> factory) {
    final Map<String, Object> rawRoot = loadRaw(source);
    return transform(rawRoot, factory);
  }

  private <T extends PropertySettable<?>> T transform(
      final Map<String, Object> rawRoot, final ObjectFactory<T> factory) {
   ...
   */
  private <T extends PropertySettable<?>> void addChild(
      final ObjectFactory<T> handler,
      final TMParserPropertyPath path,
  ...
  private void setProperty(
      final PropertySettable<?> settable, final Object propertyId, final Object value) {
