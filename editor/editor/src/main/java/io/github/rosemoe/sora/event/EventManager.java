...
public final class EventManager {
   ...
   */
  @NonNull
  public <T extends Event> SubscriptionReceipt<T> subscribe(
  ...
  public <T extends Event> SubscriptionReceipt<T> subscribeAlways(
      @NonNull Class<T> eventType, @NonNull NoUnsubscribeReceiver<T> receiver) {
  ...
  public <T extends Event> SubscriptionReceipt<T> subscribeEvent(
    ...
    try {
      ...
      if (list.contains(receiver)) {
        // Simply detect if the event receiver has been added and return the SubscriptionReceipt
        // directly.
        // Even if add multiple subscription, actually send an event, the event receiver will only
        // receive an event once.
  ...
  public <T extends Event> int dispatchEvent(@NonNull T event) {
    ...
    try {
      ...
      for (int i = 0;
          i < count && (event.getInterceptTargets() & InterceptTarget.TARGET_RECEIVERS) == 0;
          i++) {
