...
public class Paint extends android.graphics.Paint {
    ...
    if (fast) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        if (Character.isHighSurrogate(ch)
          ...
          charWidth =
              widths.measureCodePoint(Character.toCodePoint(ch, chars[index + i + 1]), this);
          if (advances != null) {
      ...
      float advance =
          getTextRunAdvances(
              chars, index, count, contextIndex, contextCount, isRtl, advances, advancesIndex);
  ...
  public float measureTextRunAdvance(
    ...
    return myGetTextRunAdvances(
        text, start, end - start, contextStart, contextEnd - contextStart, false, null, 0, fast);
  }
  ...
  public int findOffsetByRunAdvance(
      ContentLine text, int start, int end, float advance, boolean fast) {
    if (fast) {
      ensureCacheObject();
      var width = 0f;
      ...
      for (int i = start; i < end; i++) {
        ...
        if (Character.isHighSurrogate(ch)
            && i + 1 < end
            && Character.isLowSurrogate(textChars[i + 1])) {
  ...
  private int breakTextImpl(ContentLine text, int start, int end, float advance) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      return getOffsetForAdvance(
          text.getBackingCharArray(), start, end, start, end, false, advance);
    } else {
