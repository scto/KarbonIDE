...
public final class EditorTouchEventHandler
  ...
  public boolean shouldDrawScrollBarForTouch() {
    return System.currentTimeMillis() - timeLastScroll < HIDE_DELAY + SCROLLBAR_FADE_ANIMATION_TIME
    ...
    switch (e.getAction()) {
      case MotionEvent.ACTION_DOWN:
        {
          ...
          if (holdingScrollbarVertical || holdingScrollbarHorizontal) {
            ...
            if (shouldDrawInsertHandle()
                && RectUtils.almostContains(
                    editor.getInsertHandleDescriptor().position,
                    e.getX(),
                    e.getY(),
                    allowedDistance)) {
            ...
            boolean left =
                RectUtils.almostContains(
                    editor.getLeftHandleDescriptor().position, e.getX(), e.getY(), allowedDistance);
            boolean right =
                RectUtils.almostContains(
                    editor.getRightHandleDescriptor().position,
                    e.getX(),
                    e.getY(),
                    allowedDistance);
            if (left) {
      ...
      case MotionEvent.ACTION_MOVE:
        {
          if (holdingScrollbarVertical) {
            ...
            float dy =
                movedDis
                    / (editor.getHeight()
                        - editor.getRenderer().getVerticalScrollBarRect().height())
                    * all;
            scrollBy(0, dy);
            return true;
          }
          if (holdingScrollbarHorizontal) {
            ...
            if (editor.getRenderer().getHorizontalScrollBarRect().width()
              ...
              dx =
                  movedDis
                      / (editor.getWidth()
                          - editor.getRenderer().getHorizontalScrollBarRect().width())
                      * all;
            } else {
          ...
          if (!selHandleMoving
              && (Math.abs(e.getX() - thumbDownX) > touchSlop
                  || Math.abs(e.getY() - thumbDownY) > touchSlop)) {
            selHandleMoving = true;
          }
          if (selHandleMoving && handleSelectionChange(e)) {
            if (magnifier.isShowing()
                || Math.sqrt(
                        (e.getX() - thumbDownX) * (e.getX() - thumbDownX)
                            + (e.getY() - thumbDownY) * (e.getY() - thumbDownY))
                    >= MAGNIFIER_TOUCH_SLOP) {
              updateMagnifier(e);
            }
            editor.invalidate();
            return true;
      ...
      case MotionEvent.ACTION_CANCEL:
        ...
        if (selHandleType != -1) {
          ...
          if (selHandleType == SelectionHandle.BOTH) notifyLater();
          selHandleType = -1;
        }
        editor.invalidate();
        stopEdgeScroll();
    ...
    switch (event.getActionMasked()) {
      case MotionEvent.ACTION_DOWN -> {
        ...
        if ((mouseDownButtonState & MotionEvent.BUTTON_PRIMARY) != 0) {
          ...
          if (SystemClock.uptimeMillis() - lastTimeMousePrimaryClickUp
          ...
          if (editor.isTextSelected()
              && editor.getCursorRange().isPositionInside(charPos)
      ...
      case MotionEvent.ACTION_MOVE -> {
        ...
        if (Math.abs(event.getX() - mouseDownX) > touchSlop
            || Math.abs(event.getY() - mouseDownY) > touchSlop) {
          mouseClick = false;
        }
        if ((mouseDownButtonState & MotionEvent.BUTTON_PRIMARY) != 0) {
          ...
          if (!mouseClick && !mouseCanMoveText) {
            ...
            editor.setSelectionRegion(
                anchor.line, anchor.column, line, column, SelectionChangeEvent.CAUSE_MOUSE_INPUT);
          }
          draggingSelection = charPos;
          editor.postInvalidate();
      ...
      case MotionEvent.ACTION_UP -> {
        ...
        if (!mouseDoubleClick) {
          if (mouseCanMoveText
              && !mouseClick
              && (mouseDownButtonState & MotionEvent.BUTTON_PRIMARY) != 0) {
            var pos = editor.getPointPositionOnScreen(event.getX(), event.getY());
            int line = IntPair.getFirst(pos), column = IntPair.getSecond(pos);
            var dest = editor.getText().getIndexer().getCharPosition(line, column);
            var curRange = editor.getCursorRange();
            if (!curRange.isPositionInside(dest)
                && (editor.getKeyMetaStates().isCtrlPressed() || !curRange.getEnd().equals(dest))) {
              int length = (curRange.getEndIndex() - curRange.getStartIndex());
              int insIndex =
                  editor.getKeyMetaStates().isCtrlPressed()
                      ? dest.index
                      : dest.index < curRange.getStartIndex() ? dest.index : dest.index - length;
  ...
  public void onContextClick(MotionEvent event) {
    ...
    if ((dispatchEditorMotionEvent(ContextClickEvent::new, null, event)
  ...
  int dispatchEditorMotionEvent(
      Function5<CodeEditor, CharPosition, MotionEvent, Span, TextRange, EditorMotionEvent>
          constructor,
      @Nullable CharPosition pos,
    ...
    if (pos == null) {
      ...
      pos =
          editor
              .getText()
              .getIndexer()
              .getCharPosition(IntPair.getFirst(pt), IntPair.getSecond(pt));
    }
    ...
    if (span != null) {
      var startPos =
          text.getIndexer()
              .getCharPosition(
                  pos.line, Numbers.coerceIn(span.getColumn(), 0, text.getColumnCount(pos.line)));
      var endPos =
          nextSpan != null
              ? text.getIndexer()
                  .getCharPosition(
                      pos.line,
                      Numbers.coerceIn(nextSpan.getColumn(), 0, text.getColumnCount(pos.line)))
  ...
  public boolean onSingleTapUp(@NonNull MotionEvent e) {
    ...
    if ((dispatchEditorMotionEvent(ClickEvent::new, position, e) & InterceptTarget.TARGET_EDITOR)
        != 0) {
      return true;
    }
  ...
  public void onLongPress(@NonNull MotionEvent e) {
    ...
    int line = IntPair.getFirst(res);
    int column = IntPair.getSecond(res);
    if ((dispatchEditorMotionEvent(
                LongPressEvent::new, editor.getText().getIndexer().getCharPosition(line, column), e)
            & InterceptTarget.TARGET_EDITOR)
        != 0) {
      return;
    }
    if ((!editor.getProps().reselectOnLongPress && editor.getCursor().isSelected())
        || e.getPointerCount() != 1) {
      return;
    }
  ...
  final class SelectionHandle {
    ...
    public void applyPosition(MotionEvent e) {
      ...
      if (line >= 0 && line < editor.getLineCount()) {
        ...
        int lastLine =
            type == RIGHT ? editor.getCursor().getRightLine() : editor.getCursor().getLeftLine();
        int lastColumn =
            type == RIGHT
  ...
  private class EdgeScrollRunnable implements Runnable {
    ...
    public void run() {
      ...
      // Update selection
      if (thumbMotionRecord != null) handleSelectionChange2(thumbMotionRecord);
