...
public class Content implements CharSequence {
  ...
  /**
   * Create a Content object with the given content text. Specify whether thread-safe access to
  ...
  public String substring(int start, int end) {
    ...
    try {
      ...
      return subStringBuilder(
              s.getLine(), s.getColumn(), e.getLine(), e.getColumn(), end - start + 1)
          .toString();
    } finally {
      unlock(false);
    }
  ...
  /**
   * Get raw data of line. The result should not be modified by code out of editor framework.
   *
   * @param line Line
  ...
  /**
   * Check if the given {@link CharPosition} is valid in this text. Checks include line, column and
   * index.
   *
  ...
  private void insertInternal(int line, int column, CharSequence text) {
    ...
    while (true) {
      ...
      if (type == InsertTextHelper.TYPE_LINE_CONTENT) {
        ...
        var separator =
            LineSeparator.fromSeparatorString(text, helper.getIndex(), helper.getIndexNext());
        ...
        var newLine =
            new ContentLine(
                currLine.length() - workIndex + helper.getIndexNext() - helper.getIndex() + 10);
   ...
   *
   * <p>NOTE: When there are too much modification, old modification will be deleted from
   * UndoManager
   */
  ...
  private StringBuilder subStringBuilder(
      int startLine, int startColumn, int endLine, int endColumn, int length) {
    ...
    if (startLine == endLine) {
      ...
      for (int i = startLine + 1; i < endLine; i++) {
        var line = lines.get(i);
        sb.append(line).append(line.getLineSeparator().getContent());
      }
      var end = lines.get(endLine);
      if (endColumn == end.length() + 1 && end.getLineSeparator() == LineSeparator.CRLF) {
        sb.append(end, 0, endColumn).append(LineSeparator.CR.getContent());
      } else {
   ...
   *
   * <p>This can improve the speed in char reading for tokenizing
   *
   ...
   * as this object. By default, the object is thread-safe and access operations are locked when
  ...
  /**
   * Shallow copy text in this Content object. Returns a new Content object with the same text as
   * this object. By default, the object is not thread-safe and should be accessed by a single
   * thread.
   */
  ...
  /**
   * Release this text object. Release any shareable instance currently held. It's recommended to
   * call this after a shallow copied instance is no longer used.
   */
  public void release() {
    lock(true);
    try {
