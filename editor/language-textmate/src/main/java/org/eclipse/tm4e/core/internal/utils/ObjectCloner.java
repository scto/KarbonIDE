...
public final class ObjectCloner {
  ...
  private static final WeakHashMap<Class<?>, Optional<Method>> CLONE_METHODS_CACHE =
      new WeakHashMap<>();
  ...
  @Nullable
  private static <@Nullable T> T deepCloneNullable(
      final T obj, final Map<Object, @Nullable Object> clones) {
    if (obj == null) {
      return null;
    }
    ...
    if (obj instanceof Cloneable) {
      try {
        final var cloneMethod =
            CLONE_METHODS_CACHE.computeIfAbsent(
                obj.getClass(),
                cls -> {
                  try {
