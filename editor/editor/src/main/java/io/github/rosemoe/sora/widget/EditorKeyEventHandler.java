...
public class EditorKeyEventHandler {
   ...
   * EditorKeyEventHandler#keyMetaStates} must be notified about the key event before this method is
   * called.
   *
  ...
  private Boolean handleKeyEvent(
    ...
    if (userKeyEventHandler != null) {
      if (userKeyEventHandler.onKeyEvent(
          editor,
          event,
          editorKeyEvent,
          keybindingEvent,
      ...
      case KeyEvent.KEYCODE_ENTER:
        {
          return handleEnterKeyEvent(
              editorKeyEvent, keybindingEvent, isShiftPressed, isAltPressed, isCtrlPressed);
        }
      case KeyEvent.KEYCODE_DPAD_DOWN:
        if (isCtrlPressed) {
          if (isShiftPressed) {
            ...
            editorText.delete(
                right.line, editorText.getColumnCount(right.line), right.line + 1, next.length());
            ...
            final var newRight =
                editorText.getIndexer().getCharPosition(right.line + 1, right.column);
  ...
  private Boolean handlePrintingKey(KeyEvent event, EditorKeyEvent editorKeyEvent, int keyCode) {
    ...
    if (charCode != 0 && editor.isEditable()) {
      if (charCode == KeyCharacterMap.HEX_INPUT
          || charCode == KeyCharacterMap.PICKER_DIALOG_INPUT) {
            ...
            var charCount =
                Character.charCount(
                    Character.codePointBefore(
                        editor.getText().getLine(cursor.getRightLine()), cursor.getRightColumn()));
            editor.setSelectionRegion(
                cursor.getRightLine(),
                cursor.getRightColumn() - charCount,
                cursor.getRightLine(),
  ...
  private Boolean handleCtrlKeyBinding(
      EditorKeyEvent e, KeyBindingEvent keybindingEvent, int keyCode, boolean isShiftPressed) {
    final var editor = this.editor;
    final var connection = editor.inputConnection;
    final var editorText = editor.getText();
    final var editorCursor = editor.getCursor();
              ...
              try {
                var result =
                    handler.handleNewline(
                        editorText, editorCursor.left(), editor.getStyles(), editor.getTabWidth());
