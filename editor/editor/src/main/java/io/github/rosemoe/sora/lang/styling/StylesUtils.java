...
public class StylesUtils {
  ...
  /**
   * Check given position's span. If {@link
  ...
  public static Span getFollowingSpanForPosition(
      @Nullable Styles styles, @NonNull CharPosition pos) {
    return getSpanForPositionImpl(styles, pos, 1);
  }

  @Nullable
  private static Span getSpanForPositionImpl(
      @Nullable Styles styles, @NonNull CharPosition pos, int spanIndexOffset) {
    var line = pos.line;
    ...
    try {
      ...
      if (ex != null) Log.e(LOG_TAG, "failed to get spans from " + reader + " at " + pos, ex);
    }
  }
}
