...
public class SymbolPairMatch {
   ...
   * replaced by {@param symbolPair} SymbolPair maybe null to disable completion for this character.
   *
   * @see SymbolPair
   */
  ...
  /**
   * Put a pair of symbol completion When the user types the {@param charArray}, it will be replaced
   * by {@param symbolPair} SymbolPair maybe null to disable completion for this character.
   *
   * @see SymbolPair
   */
  ...
  public final SymbolPair matchBestPair(
      CodeEditor editor, CharPosition cursorPosition, char[] inputCharArray, char firstChar) {
  ...
  public static class SymbolPair {
    ...
    /**
     * If your {@param open} string and {@param close} string are both ', it makes a pair of single
     * quotes. This will replace the entered character with a pair of single quotes, and will move
     * the cursor to the middle of the pair. This class defines these symbol pairs
     */
    ...
    public interface SymbolPairEx {
      /**
       * The method will be called to decide whether to perform the replacement or not. It may be
       * same as vscode language-configuration Auto-closing 'notIn'. also see <a
       ...
       *
       * @param editor The current edit content, sometimes you may need to get the analyzed data
       *     from {@link AnalyzeManager} (e.g. token with tags) and use editor to get more
       *     information, such as the line of the cursor.
       * @param currentLine The current line edit in the editor,quick analysis it to decide whether
       *     to replaced
      ...
      /**
       * when before the replaced and select a range,surrounds the selected content with return
       * ture. If not implemented, always return false also see <a
