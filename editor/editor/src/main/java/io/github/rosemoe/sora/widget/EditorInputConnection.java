...
class EditorInputConnection extends BaseInputConnection {
  ...
  private CharSequence getTextRegionInternal(
      int start, int end, int flags, boolean ignoreIPCLimit) {
    ...
    if (editor.getProps().trackComposingTextOnCommit) {
      if (composingText.isComposing()) {
        var composingText =
            editor
                .getText()
                .subSequence(this.composingText.startIndex, this.composingText.endIndex)
                .toString();
  ...
  private void setComposingTextCompat(@NonNull String text) {
    ...
    if (current.length() < text.length() && text.startsWith(current)) {
      ...
      content.delete(
          composingText.endIndex - (current.length() - text.length()), composingText.endIndex);
    } else {
  ...
  public SurroundingText getSurroundingText(int beforeLength, int afterLength, int flags) {
    ...
    var text =
        getTextRegionUnlimited(
            startOffset,
            Math.min(editor.getText().length(), getCursor().getRight() + afterLength),
            flags);
