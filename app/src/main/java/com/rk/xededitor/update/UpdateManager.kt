...
object UpdateManager {
    ...
    fun fetch(branch: String) {
        try {
            GlobalScope.launch(Dispatchers.IO) {
                ...
                val lastUpdate =
                    PreferencesData.getString(PreferencesKeys.LAST_UPDATE_CHECK, "0").toLong()
                ...
                PreferencesData.setString(
                    PreferencesKeys.LAST_UPDATE_CHECK,
    ...
    private suspend inline fun parseJson(jsonStr: String) {
        ...
        for (i in 0 until jsonArray.length()) {
            ...
            val author =
                latestCommit.getJSONObject("commit").getJSONObject("author").getString("name")
            ...
            val date =
                latestCommit.getJSONObject("commit").getJSONObject("author").getString("date")

            if (
                convertToUnixTimestamp(BuildConfig.GIT_COMMIT_DATE) < convertToUnixTimestamp(date)
            ) {
