...
public class SyncRegistry implements IGrammarRepository, IThemeProvider {
  ...
  public void addGrammar(
      final IRawGrammar grammar, @Nullable final Collection<String> injectionScopeNames) {
  ...
  public @Nullable IRawGrammar lookup(final String scopeName) {
    ...
    if (grammar == null) {
      ...
      // if no grammar was found for the given scopeName, check if the scopeName is qualified, e.g.
      // "source.mylang@com.example.mylang.plugin"
      // and if so try the unqualified scopeName "source.mylang" as fallback. See
      // org.eclipse.tm4e.registry.internal.TMScope and
      // for more details
      // org.eclipse.tm4e.registry.internal.AbstractGrammarRegistryManager.getGrammarFor(IContentType...)
