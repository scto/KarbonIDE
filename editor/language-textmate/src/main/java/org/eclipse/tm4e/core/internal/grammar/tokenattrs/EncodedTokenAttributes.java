...
public final class EncodedTokenAttributes {
  ...
  public static int getLanguageId(final int metadata) {
    return (metadata & EncodedTokenDataConsts.LANGUAGEID_MASK)
        >>> EncodedTokenDataConsts.LANGUAGEID_OFFSET;
  }

  public static int getTokenType(final int metadata) {
    return (metadata & EncodedTokenDataConsts.TOKEN_TYPE_MASK)
  ...
  public static int getFontStyle(final int metadata) {
    return (metadata & EncodedTokenDataConsts.FONT_STYLE_MASK)
        >>> EncodedTokenDataConsts.FONT_STYLE_OFFSET;
  }

  public static int getForeground(final int metadata) {
    return (metadata & EncodedTokenDataConsts.FOREGROUND_MASK)
        >>> EncodedTokenDataConsts.FOREGROUND_OFFSET;
  }

  public static int getBackground(final int metadata) {
    return (metadata & EncodedTokenDataConsts.BACKGROUND_MASK)
        >>> EncodedTokenDataConsts.BACKGROUND_OFFSET;
  }

  /**
   * Updates the fields in `metadata`. A value of `0`, `NotSet` or `null` indicates that the
   * corresponding field should be left as is.
   */
  public static int set(
      final int metadata,
      final int languageId,
      final /*OptionalStandardTokenType*/ int tokenType,
      @Nullable final Boolean containsBalancedBrackets,
      final int fontStyle,
      final int foreground,
      final int background) {
    final var _languageId = languageId == 0 ? getLanguageId(metadata) : languageId;
    final var _tokenType =
        tokenType == OptionalStandardTokenType.NotSet ? getTokenType(metadata) : tokenType;
    final var _containsBalancedBracketsBit =
        (containsBalancedBrackets == null
