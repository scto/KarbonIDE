 ...
 *
 * <p>Sub-classes must implement the method <code>dispatchEvent(LISTENER, EVENT)</code> which is
 * responsible for invoking the appropriate event handler method on the listener.
 *
...
public abstract class AbstractListeners<LISTENER, EVENT> {
  ...
  public boolean add(final LISTENER listener) {
    // LOGGER.log(DEBUG, "Trying to add listener {0} which is already registered with {1}.",
    // listener, this);
  ...
  public boolean remove(final LISTENER listener) {
    // LOGGER.log(WARNING, "Trying to remove listener {0} which is not registered with {1}.",
    // listener, this);
