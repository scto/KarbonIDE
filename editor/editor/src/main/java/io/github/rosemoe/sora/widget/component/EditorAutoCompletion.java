...
public class EditorAutoCompletion extends EditorPopupWindow implements EditorBuiltinComponent {
  ...
  public EditorAutoCompletion(@NonNull CodeEditor editor) {
    ...
    eventManager.subscribeEvent(
        EditorReleaseEvent.class, (event, unsubscribe) -> setEnabled(false));
  ...
  protected <T extends Event> void subscribeEventForHide(
      Class<T> clazz, Function1<T, Boolean> predicate) {
    eventManager.subscribeEvent(
        clazz,
        (event, unsubscribe) -> {
  ...
  protected void onColorSchemeUpdate(
      @NonNull ColorSchemeUpdateEvent event, @NonNull Unsubscribe unsubscribe) {
    applyColorScheme();
  }

  protected void onContentChange(
      @NonNull ContentChangeEvent event, @NonNull Unsubscribe unsubscribe) {
  ...
  protected void onSelectionChange(
    ...
    if (event.isSelected()
        || event.getCause() == SelectionChangeEvent.CAUSE_IME
        || event.getCause() == SelectionChangeEvent.CAUSE_SELECTION_HANDLE
        || event.getCause() == SelectionChangeEvent.CAUSE_TAP
        || event.getCause() == SelectionChangeEvent.CAUSE_SEARCH
        || event.getCause() == SelectionChangeEvent.CAUSE_UNKNOWN) {
      hide();
      return;
    }
    ...
    if (event.getCause() == SelectionChangeEvent.CAUSE_KEYBOARD_OR_CODE) {
      ...
      if (isShowing() && Math.abs(previousSelection.column - event.getLeft().column) <= 1) {
        if (event.getLeft().column > 0) requireCompletion();
        else hide();
      }
    }
  }
  ...
  public void updateCompletionWindowPosition(boolean scrollEditor) {
    ...
    float[] rightLayoutOffset =
        editor.getLayout().getCharLayoutOffset(cursor.getRightLine(), cursor.getRightColumn());
    ...
    if (restY > dp * 200) {
      ...
      float offset = 0;
      while (restY < dp * 100
          && editor.getOffsetY() + offset + rowHeight <= editor.getScrollMaxY()) {
        restY += rowHeight;
        panelY -= rowHeight;
        offset += rowHeight;
      }
      editor
          .getScroller()
          .startScroll(editor.getOffsetX(), editor.getOffsetY(), 0, (int) offset, 0);
    }
    int width;
    if ((editor.getWidth() < 500 * dp && completionWndPosMode == WINDOW_POS_MODE_AUTO)
  ...
  /**
   * Set adapter for auto-completion window This will take effect next time the window updates
   *
