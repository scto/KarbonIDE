...
public final class ThemeTrieElement {
  ...
  public ThemeTrieElement(
      final ThemeTrieElementRule mainRule, final List<ThemeTrieElementRule> rulesWithParentScopes) {
  ...
  private static List<ThemeTrieElementRule> _sortBySpecificity(
      final List<ThemeTrieElementRule> arr) {
    if (arr.size() == 1) {
      return arr;
    }
  ...
  public List<ThemeTrieElementRule> match(final String scope) {
    if ("".equals(scope)) {
      return ThemeTrieElement._sortBySpecificity(
          asArrayList(this._mainRule, this._rulesWithParentScopes));
    }
  ...
  public void insert(
      ...
      final String scope,
      @Nullable final List<String> parentScopes,
      final int fontStyle,
    ...
    if (this._children.containsKey(head)) {
      ...
      child =
          new ThemeTrieElement(
              this._mainRule.clone(), ThemeTrieElementRule.cloneArr(this._rulesWithParentScopes));
