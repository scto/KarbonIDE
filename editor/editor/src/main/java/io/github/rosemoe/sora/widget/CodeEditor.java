...
public class CodeEditor extends View implements ContentListener, Formatter.FormatResultReceiver {
  ...
  public void setKeyEventHandler(
      io.github.rosemoe.sora.interfaces.KeyEventHandler UserkeyEventHandler) {
  ...
  protected void applyAttributeSets(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    var array =
        getContext()
            .obtainStyledAttributes(attrs, R.styleable.CodeEditor, defStyleAttr, defStyleRes);

    setHorizontalScrollbarThumbDrawable(
        array.getDrawable(R.styleable.CodeEditor_scrollbarThumbHorizontal));
    setHorizontalScrollbarTrackDrawable(
        array.getDrawable(R.styleable.CodeEditor_scrollbarTrackHorizontal));
    setVerticalScrollbarThumbDrawable(
        array.getDrawable(R.styleable.CodeEditor_scrollbarThumbVertical));
    setVerticalScrollbarTrackDrawable(
        array.getDrawable(R.styleable.CodeEditor_scrollbarTrackVertical));
    ...
    setLineNumberEnabled(
        array.getBoolean(R.styleable.CodeEditor_lineNumberVisible, isLineNumberEnabled()));
    getComponent(EditorAutoCompletion.class)
        .setEnabled(array.getBoolean(R.styleable.CodeEditor_autoCompleteEnabled, true));
    props.symbolPairAutoCompletion =
        array.getBoolean(R.styleable.CodeEditor_symbolCompletionEnabled, true);
    setRenderFunctionCharacters(
        array.getBoolean(R.styleable.CodeEditor_renderFunctionChars, isRenderFunctionCharacters()));
    ...
    setCursorBlinkPeriod(
        array.getInt(R.styleable.CodeEditor_cursorBlinkPeriod, getCursorBlink().period));
   ...
   * @param selectionOffset New selection position relative to the start of text to insert. Ranging
  ...
  /**
   * This only makes sense when wordwrap is enabled. Checks if anti word breaking is enabled in
   * wordwrap mode.
   */
  ...
  protected long findLeadingAndTrailingWhitespacePos(ContentLine line) {
    ...
    if (leading != column
        && (nonPrintableOptions & (FLAG_DRAW_WHITESPACE_INNER | FLAG_DRAW_WHITESPACE_TRAILING))
            != 0) {
  ...
  public void unindentSelection() {
    ...
    for (int i = cursor.getLeftLine(); i <= cursor.getRightLine(); i++) {
      ...
      if (spaceCount > 0 && tabCount > 0) {
        ...
        final var finalSpaceCount =
            Math.abs(spaces - (extraSpaces == 0 ? tabWidth : extraSpaces)) / tabWidth;
        text.replace(i, 0, i, endColumn, StringsKt.repeat(tabString, finalSpaceCount));
        continue;
      }
  ...
  /**
   * Indents the line if text is not selected and the cursor is at the start of the line. Inserts an
   * indentation string otherwise.
   */
  ...
  public void deleteText() {
    ...
    if (cur.isSelected()) {
      ...
      if (props.deleteEmptyLineFast
          || (props.deleteMultiSpaces != 1 && col > 0 && text.charAt(line, col - 1) == ' ')) {
  ...
  public void commitText(CharSequence text, boolean applyAutoIndent) {
    ...
    if (getProps().symbolPairAutoCompletion && text.length() > 0) {
      ...
      pair = languageSymbolPairs.matchBestPair(this, cursor.left(), inputText, firstCharFromText);
    }
    ...
    if (pair != null && pair != SymbolPairMatch.SymbolPair.EMPTY_SYMBOL_PAIR) {
      ...
      // the AutoSurround feature is not considered needed because it can be implemented through
      // QuickQuoteHandler
      if (pair.shouldDoAutoSurround(editorText) && quoteHandler == null) {
        ...
        setSelectionRegion(
            ...
            cur.getLeftColumn(),
            cur.getRightLine(),
            cur.getRightColumn() - pair.close.length());
        ...
        var insertPosition = editorText.getIndexer().getCharPosition(pair.getInsertOffset());
        ...
        editorText.replace(
            ...
            pair.open);
        editorText.insert(
            insertPosition.line, insertPosition.column + pair.open.length(), pair.close);
        editorText.endBatchEdit();

        var cursorPosition = editorText.getIndexer().getCharPosition(pair.getCursorOffset());
    ...
    if (cur.isSelected()) {
      editorText.replace(
          cur.getLeftLine(), cur.getLeftColumn(), cur.getRightLine(), cur.getRightColumn(), text);
    } else {
  ...
  public boolean isScreenPointOnText(float x, float y) {
    ...
    var rowIdx =
        layout.getRowIndexForPosition(
            text.getCharIndex(IntPair.getFirst(pos), IntPair.getSecond(pos)));
    ...
    if (stuckLines != null) {
      if (y < stuckLines.size() * getRowHeight()) {
        ...
        return getPointPosition(
            x,
            layout.getCharLayoutOffset(stuckLines.get(index).startLine, 0)[0]
                - getRowHeight() / 2f);
      }
    }
   ...
   * 3. Some emojis with variation selector or fitzpatrick can not be shown correctly with specified
   * attributes.<br>
            ...
            new AlertDialog.Builder(getContext())
                .setTitle(
                    I18nConfig.getResourceId(
                        replaceAll
                            ? R.string.sora_editor_replaceAll
                            : R.string.sora_editor_replace))
                .setView(et)
  ...
  public int getLastVisibleRow() {
    return Math.max(
        0, Math.min(layout.getRowCount() - 1, (getOffsetY() + getHeight()) / getRowHeight()));
  }
   ...
   *
   * @param add The value in pixels that should be added to each line other than the last line. This
   *     will be applied after the multiplier
   * @param mult The value by which each line height other than the last line will be multiplied by
   */
  ...
  /**
   * @param lineSpacingMultiplier The value by which each line height other than the last line will
   *     be multiplied * by. Default 1.0f
   */
  ...
  public int getLineSpacingPixels() {
    ...
    return ((int)
            ((metrics.descent - metrics.ascent) * (lineSpacingMultiplier - 1f) + lineSpacingAdd))
        / 2
        * 2;
  }
  ...
  public void pasteText() {
    try {
      ...
      if (!clipboardManager.hasPrimaryClip()
          || (clip = clipboardManager.getPrimaryClip()) == null) {
        return;
      }
  ...
  public void selectWord(int line, int column) {
    ...
    setSelectionRegion(
        start.line, start.column, end.line, end.column, SelectionChangeEvent.CAUSE_LONG_PRESS);
  }
   ...
   *
   * @param text the new text you want to display
   * @param extraArguments Extra arguments for the document. This {@link Bundle} object is passed to
   *     all languages and plugins in editor.
   */
   ...
   */
  public void setText(
      @Nullable CharSequence text, boolean reuseContentObject, @Nullable Bundle extraArguments) {
    if (text == null) {
      text = "";
    }
    ...
    dispatchEvent(
        new ContentChangeEvent(
            ...
            this.text
                .getIndexer()
                .getCharPosition(getLineCount() - 1, this.text.getColumnCount(getLineCount() - 1)),
            this.text,
            false));
    createLayout();
   ...
   * @see EventManager#subscribeEvent(Class, EventReceiver)
   */
  public <T extends Event> SubscriptionReceipt<T> subscribeAlways(
      Class<T> eventType, EventManager.NoUnsubscribeReceiver<T> receiver) {
  ...
  /**
   * Create a new {@link EventManager} instance that can be used to subscribe events in editor, as a
   * child instance of editor.
   *
  ...
  /**
   * Mark current selection position as a point of cursor range. When user taps to select another
   * point in text, the text between the marked point and newly chosen point is selected.
   *
   * @see #isInLongSelect()
   * @see #endLongSelect()
   */
  ...
  /** Marks long select mode is end. This does nothing but set the flag to false. */
  ...
  /**
   * Check whether the soft keyboard is enabled for this editor. Unlike {@link
   * #isSoftKeyboardEnabled()}, this method also checks whether a hardware keyboard is connected.
   *
   ...
   *
   * @param isDisabled Whether the soft keyboard should be enabled if hardware keyboard is
   *     connected.
   */
  ...
  /**
   * Release some resources held by editor. This will stop completion threads and destroy using
   * {@link Language} object. Also it prevents future editor tasks (such as posted Runnable) to be
   * executed.
   *
   * <p>You are expected to call this method when the editor instance is no longer used, especially
   * when your activity is to be destroyed. Invoking this method repeatedly will not generated
   * errors.
   */
  public void release() {
    hideEditorWindows();
  ...
  /**
   * Check if the editor is released. When an editor is released, you are unexpected to make any
   * changes to it.
   */
  ...
  public boolean onTouchEvent(MotionEvent event) {
    ...
    if (event.isFromSource(InputDevice.SOURCE_MOUSE)
        && props.mouseMode != DirectAccessProps.MOUSE_MODE_NEVER) {
  ...
  protected void onCreateContextMenu(ContextMenu menu) {
    ...
    dispatchEvent(
        new CreateContextMenuEvent(
            this,
            menu,
            text.getIndexer()
                .getCharPosition(IntPair.getFirst(charPos), IntPair.getSecond(charPos))));
  }
  ...
  public boolean onGenericMotionEvent(MotionEvent event) {
    ...
    if (event.getAction() == MotionEvent.ACTION_SCROLL
        ...
        && !keyEventHandler.getKeyMetaStates().isCtrlPressed()) {
        ...
        () -> {
          ...
          var string =
              (applyContent instanceof Content)
                  ...
                  : applyContent;
          text.beginBatchEdit();
          text.delete(0, 0, text.getLineCount() - 1, text.getColumnCount(text.getLineCount() - 1));
