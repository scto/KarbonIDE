...
final class LineTokenizer {
  ...
  private void scanNext() {
    ...
    final MatchResult r =
        matchRuleOrInjections(grammar, lineText, isFirstLine, linePos, stack, anchorPosition);
    ...
    if (matchedRuleId.equals(RuleId.END_RULE)) {
      ...
      if (!hasAdvanced && popped.getEnterPos() == linePos) {
        ...
        // LOGGER.log(INFO, "[1] - Grammar is in an endless loop - Grammar pushed & popped a rule
        // without advancing");
      ...
      final var nameScopesList =
          castNonNull(stack.contentNameScopesList).pushAttributed(scopeName, grammar);
      stack =
          stack.push(
              matchedRuleId,
              linePos,
      ...
      if (rule instanceof final BeginEndRule pushedRule) {
        ...
        if (!hasAdvanced && beforePush.hasSameRuleAs(stack)) {
          ...
          // LOGGER.log(INFO, "[2] - Grammar is in an endless loop - Grammar pushed the same rule
          // without advancing");
          stack = castNonNull(stack.pop());
          lineTokens.produce(stack, lineText.content.length());
          stop = true;
        ...
        if (!hasAdvanced && beforePush.hasSameRuleAs(stack)) {
          // Grammar pushed the same rule without advancing
          // LOGGER.log(INFO, "[3] - Grammar is in an endless loop - Grammar pushed the same rule
          // without advancing");
          stack = castNonNull(stack.pop());
          lineTokens.produce(stack, lineText.content.length());
          stop = true;
        ...
        if (!hasAdvanced) {
          ...
          // LOGGER.log(INFO, "[4] - Grammar is in an endless loop - Grammar is not advancing, nor
          // is it pushing/popping");
          stack = stack.safePop();
          lineTokens.produce(stack, lineText.content.length());
          stop = true;
          return;
  ...
  private MatchResult matchRule(
    ...
    final var ruleScanner =
        rule.compileAG(grammar, stack.endRule, isFirstLine, linePos == anchorPosition);
  ...
  @Nullable
  private MatchResult matchRuleOrInjections(
      final Grammar grammar,
      final OnigString lineText,
      final boolean isFirstLine,
      final int linePos,
  ...
  @Nullable
  private MatchInjectionsResult matchInjections(
    ...
    final List<String> scopes =
        stack.contentNameScopesList != null
    ...
    for (int i = 0, len = injections.size(); i < len; i++) {
      ...
      if (matchRating > bestMatchRating) {
        // Injections are sorted by priority, so the previous injection had a better or equal
        // priority
        continue;
      }
    ...
    if (bestMatchCaptureIndices != null) {
      return new MatchInjectionsResult(
          bestMatchRuleId, bestMatchCaptureIndices, bestMatchResultPriority == -1);
    }
  ...
  private void handleCaptures(
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (captureRuleScopeName != null) {
        // push
        final var base =
            localStack.isEmpty() ? stack.contentNameScopesList : localStack.getLast().scopes;
        final var captureRuleScopesList =
            castNonNull(base).pushAttributed(captureRuleScopeName, grammar);
  ...
  /**
   * Walk the stack from bottom to top, and check each while condition in this order. If any fails,
   * cut off the entire stack above the failed while condition. While conditions may also advance
   * the linePosition.
   */
  ...
  static TokenizeStringResult tokenizeString(
      final Grammar grammar,
      final OnigString lineText,
      final boolean isFirstLine,
      final int linePos,
