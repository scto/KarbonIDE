...
public class MappedSpanUpdater {
  ...
  public static void shiftSpansOnMultiLineDelete(
    ...
    while (endLineSpans.size() > 1) {
      if (endLineSpans.get(0).getColumn() <= startColumn
          && endLineSpans.get(1).getColumn() <= startColumn) {
  ...
  public static void shiftSpansOnSingleLineInsert(
    ...
    if (originIndex == 0) {
      ...
      if (first.getColumn() == EditorColorScheme.TEXT_NORMAL
          && first.hasSpanExt(SpanExtAttrs.EXT_UNDERLINE_COLOR)) {
        first.setColumn(0);
      } else {
        spanList.add(0, SpanFactory.obtain(0, EditorColorScheme.TEXT_NORMAL));
      }
  ...
  public static void shiftSpansOnMultiLineInsert(
    ...
    if (endLineSpans.size() > 1
        && endLineSpans.get(0).getColumn() == 0
        && endLineSpans.get(1).getColumn() == 0) {
      endLineSpans.remove(0).recycle();
    }
  }
