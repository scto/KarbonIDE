...
public final class ScopeDependencyProcessor {
  ...
  private void collectExternalReferencesInTopLevelRepositoryRule(
  ...
  private void collectExternalReferencesInTopLevelRule(
      final Context context, final ExternalReferenceCollector result) {
  ...
  private void collectExternalReferencesInRules(
    ...
    for (final var rule : rules) {
      ...
      if (patternPatterns != null) {
        collectExternalReferencesInRules(
            patternPatterns, new ContextWithRepository(context, patternRepository), result);
      }
      ...
      switch (reference.kind) {
        case Base:
          collectExternalReferencesInTopLevelRule(
              new Context(context.baseGrammar, context.baseGrammar), result);
          break;
        case Self:
        ...
        case RelativeReference:
          collectExternalReferencesInTopLevelRepositoryRule(
              reference.ruleName, new ContextWithRepository(context, patternRepository), result);
          break;
        case TopLevelReference:
        case TopLevelRepositoryReference:
          @Nullable
          final IRawGrammar selfGrammar =
              reference.scopeName.equals(context.selfGrammar.getScopeName())
                  ? context.selfGrammar
          ...
          if (selfGrammar != null) {
            final var newContext =
                new ContextWithRepository(context.baseGrammar, selfGrammar, patternRepository);
            if (reference.kind == IncludeReference.Kind.TopLevelRepositoryReference) {
              collectExternalReferencesInTopLevelRepositoryRule(
                  reference.ruleName, newContext, result);
            } else {
            ...
            if (reference.kind == IncludeReference.Kind.TopLevelRepositoryReference) {
              result.add(
                  new TopLevelRepositoryRuleReference(reference.scopeName, reference.ruleName));
            } else {
