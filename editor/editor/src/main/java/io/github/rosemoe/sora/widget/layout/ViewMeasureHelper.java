...
public class ViewMeasureHelper {
  ...
  public static long getDesiredSize(
      ...
      if (View.MeasureSpec.getMode(widthMeasureSpec) != View.MeasureSpec.EXACTLY) {
        var lines =
            View.MeasureSpec.getMode(heightMeasureSpec) != View.MeasureSpec.EXACTLY
                ? new int[text.getLineCount()]
                : null;
        var lineMaxSize = new MutableInt(0);
        text.runReadActionsOnLines(
            0,
            text.getLineCount() - 1,
            (Content.ContentLineConsumer)
                (index, line, directions) -> {
                  int measured =
                      (int)
                          Math.ceil(
                              measurer.measureText(
                                  line.getBackingCharArray(), 0, line.length(), paint));
