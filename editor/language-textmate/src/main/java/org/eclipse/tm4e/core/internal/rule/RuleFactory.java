...
public final class RuleFactory {
  ...
  private static CaptureRule createCaptureRule(
      final IRuleFactoryHelper helper,
  ...
  public static RuleId getCompiledRuleId(
          ...
          ruleId -> {
            ...
            if (ruleWhile != null) {
              return new BeginWhileRule(
                  ...
                  begin,
                  _compileCaptures(
                      defaultIfNull(desc.getBeginCaptures(), desc.getCaptures()),
                      helper,
                      repository),
                  ruleWhile,
                  _compileCaptures(
                      defaultIfNull(desc.getWhileCaptures(), desc.getCaptures()),
                      helper,
                      repository),
            ...
            return new BeginEndRule(
                ...
                begin,
                _compileCaptures(
                    defaultIfNull(desc.getBeginCaptures(), desc.getCaptures()), helper, repository),
                desc.getEnd(),
                _compileCaptures(
                    defaultIfNull(desc.getEndCaptures(), desc.getCaptures()), helper, repository),
  ...
  private static List<@Nullable CaptureRule> _compileCaptures(
      @Nullable final IRawCaptures captures,
      final IRuleFactoryHelper helper,
    ...
    for (final String captureId : captures.getCaptureIds()) {
      ...
      final RuleId retokenizeCapturedWithRuleId =
          rule.getPatterns() == null
              ? RuleId.NO_RULE
              : getCompiledRuleId(captures.getCapture(captureId), helper, repository);
      r.set(
          numericCaptureId,
          createCaptureRule(
              helper, rule.getName(), rule.getContentName(), retokenizeCapturedWithRuleId));
    }
    return r;
  }
  ...
  private static CompilePatternsResult _compilePatterns(
      @Nullable final Collection<IRawRule> patterns,
      final IRuleFactoryHelper helper,
      final IRawRepository repository) {
    if (patterns == null) {
        ...
        switch (reference.kind) {
          ...
          case RelativeReference:
            ...
            if (localIncludedRule != null) {
              ...
              LOGGER.w(
                  "CANNOT find rule for scopeName [{0}]. I am [{1}]",
          ...
          case TopLevelReference, TopLevelRepositoryReference:
            ...
            if (externalGrammar != null) {
              ...
              final String externalGrammarInclude =
                  reference.kind == IncludeReference.Kind.TopLevelRepositoryReference
                      ? reference.ruleName
                      : null;
              if (externalGrammarInclude != null) {
                final var externalIncludedRule =
                    externalGrammarRepo.getRule(externalGrammarInclude);
                ...
                ruleId =
                    getCompiledRuleId(externalGrammarRepo.getSelf(), helper, externalGrammarRepo);
              }
            } else {
              LOGGER.w(
                  "CANNOT find grammar for scopeName [{0}]. I am [{1}]",
                  patternInclude, repository.getBase().getName());
            }
            break;
        }
